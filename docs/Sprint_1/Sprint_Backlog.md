...
|:---:|:---|:---|:---:|:---:|:---:|
| **US-101** | Infraestructura | Iniciar Repositorios (MonoRepo) y conectar MongoDB Atlas Cluster M0. | 2 | Paul Paytan | ✅ Completado |
| **US-102** | Seguridad Backend | Crear Schema `User` de Mongoose y cifrar las contraseñas con `bcryptjs` en los ganchos `pre-save`. | 3 | Paul Paytan | ✅ Completado |
| **US-103** | Seguridad Backend | Programar Ruta `/auth/login` que valide el hash y firme un JWT (JSON Web Token) con vencimiento de 4 horas. | 5 | Paul Paytan | ✅ Completado |
| **US-104** | Seguridad Backend | Construir un *Middleware* de Express que intercepte peticiones HTTP y extraiga el JWT del Header de Autorización. Rechazar 401 si no existe o expiró. | 3 | Paul Paytan | ✅ Completado |
