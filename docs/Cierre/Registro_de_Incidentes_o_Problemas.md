...
| **INC-006** | **Rendimiento Visual (Lag en Calendario UI):** La librería `react-big-calendar` presentaba tartamudeo visual (Jank / Caída de FPS) al intentar renderizar en pantalla un semestre entero con más de 600 bloques de colores en simultáneo. | Roberto S. (Frontend Dev) | 15/06/2026 | 18/06/2026 | **Memoización y Virtualización:** Se encapsularon los componentes de los Eventos en React utilizando `React.memo` para evitar re-renderizados inútiles. Se implementó una lógica de paginación que solo dibuja en el DOM la semana actual visible por el usuario, destruyendo los nodos ocultos. | Cerrado ✅ |
